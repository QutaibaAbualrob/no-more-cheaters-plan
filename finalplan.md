...
| June 13 | **Email config wired to env vars (Task 3.4 prep).** Replaced hardcoded `EMAIL_BACKEND = 'django.core.mail.backends.console.EmailBackend'` with env-driven settings: `EMAIL_BACKEND`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USE_TLS`, `EMAIL_USE_SSL`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `DEFAULT_FROM_EMAIL`, `SERVER_EMAIL` all read from environment via existing `env()`/`env_bool()` helpers. Defaults to console backend for zero-config local dev. Added `STATIC_ROOT` env override. Updated `.env.example` with full Resend SMTP config, database, Redis, CORS, and storage vars — copy to `.env` and fill in real values on the EC2 box. No migration needed — Django's `smtp.EmailBackend` is built-in. Resend SMTP host: `smtp.resend.com:587` (TLS) with API key as password. |
