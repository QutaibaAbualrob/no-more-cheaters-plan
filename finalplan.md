...
| June 16 | **AI detection + evidence-overlay fix (root-caused on real footage).** Investigated the report symptom "green box on an empty wall labelled `Person 1 ??? Laptop Detected`, no laptop/phone present" by running YOLO11x on the actual `school.mp4` classroom scene. Found **three stacked problems** and fixed all: **(1) Detection mis-tuning** — `ObjectDetector` ran at the 640 default (downscaling small/distant phones away) and used one global 0.40 confidence, so YOLO's reading of bright **exam paper as "laptop"** (measured false-positive confidence up to **0.79**) became real alerts. Fix: `imgsz` now configurable, default **960** (`AI_OBJECT_IMGSZ`); confidence split into a coarse predict floor (0.30) + **per-class keep thresholds** — phone **0.30**, laptop **0.85** (`AI_PHONE_CONFIDENCE`/`AI_LAPTOP_CONFIDENCE`), applied post-inference in `apis/ai/yolo_detector.py`. **(2) Wrong overlay box** (audit H6/H11) — the real detection bbox was discarded in `consolidate_events`, and the report drew an unrelated **Haar-cascade face box** (or a centred fallback) that landed on bookshelves/walls. Fix: `AlertEvent` now carries `bbox`+`bbox_timestamp_sec` (`apis/ai/detector.py`); `services._build_alert` stores `metadata['detection_bbox']=[x,y,w,h]`; `services._attach_alert_evidence` **draws that real box** (object box for phone/laptop, person box for looking-away), keeps Haar only for the avatar face-crop, gray context boxes, and **spatial** nearest-person attribution. The `???` was an em-dash unrenderable in OpenCV's font → changed `—` to `-` in `apis/ai/face_tracker.py`. **(3) Looking-away unreliable on this camera** — the rewritten v2 heuristic (mandatory shoulder-normalised horizontal nose-offset + ear-asymmetry + deep-offset votes, in `apis/ai/pose_analyzer.py`; also fixes the H11 degenerate-box merge by deriving a real person box from keypoints) flagged **7/12 forward-facing students** on the oblique/overhead documentary camera because perspective contaminates every 2D signal. Decision: gated **OFF by default** (`AI_ENABLE_LOOKING_AWAY=false`), opt-in for frontal cameras; the pose pass is skipped entirely when disabled (`apis/ai/detector.py`). `test-photos/run.py` aligned to production (per-class floors + shoulder-normalised pose ratios). **Verified:** classroom clip now yields **0 false alerts** at defaults; forcing the laptop threshold low proved `detection_bbox == drawn bbox` (box hugs the real detection, not a wall); **58/58** `apis` tests pass. Files: `apis/ai/{config,yolo_detector,detector,pose_analyzer,face_tracker}.py`, `apis/services.py`, `test-photos/run.py`. All knobs env-configurable; Alert/serializer/frontend `[x,y,w,h]` contracts and the mocked tests preserved (new event fields read via `getattr`). |
| June 17 | **AI architecture dossier added — `ai_arch/` (plan repo).** Authored a detailed 7-file write-up of the AI subsystem and the June 16 fix: `README.md` (index + executive summary), `01_pipeline_architecture.md` (full end-to-end data flow, every module/dataclass, temporal consolidation, evidence path, contracts), `02_investigation_findings.md` (empirical proof + the measured laptop/phone confidence-distribution tables + the oblique-camera looking-away negative result), `03_changes_made.md` (file-by-file before/after + the design-decisions table), `04_configuration_reference.md` (every `AI_*` env var, defaults, and camera profiles), `05_verification.md` (test results + reproduction steps), `06_conclusion_and_followups.md` (conclusion, known limitations, prioritised follow-ups: 3D head-pose model for looking-away, pose-based attribution, SAHI tiling, real-pipeline tests). Documentation only — no code changed in this entry. |
